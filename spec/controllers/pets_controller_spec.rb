...
describe "Pets Controller" do
    ...
    describe "edit action" do 
        ...
        it "edits the pet's name" do
            visit "/pets/#{@pet.id}/edit"
            #field starts as "Chewie"
            fill_in "pet_name", :with => "Chewie Darling" 
            # Checked with page.html...
            # Field has not change
            # binding.pry
            # find("#pet_name").set "Chewie Darling" # Doesn't work either.

            click_button "Update Pet"
            expect(Pet.last.name).to eq("Chewie Darling")
        end

        it "edits the pet's owner with an existing owner" do
        ...
        it "edits the pet's owner with a new owner" do
            visit "/pets/#{@pet.id}/edit"
